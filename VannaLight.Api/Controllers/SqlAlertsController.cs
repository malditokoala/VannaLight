...
public sealed record UserSqlAlertUpsertRequest(
    long Id,
    string? RuleKey,
    string TenantKey,
    string Domain,
...
public sealed record UserSqlAlertActivationRequest(bool IsActive);

[ApiController]
[Route("api/sql-alerts")]
public sealed class SqlAlertsController(
    ...
    ClearSqlAlertUseCase clearUseCase) : ControllerBase
{
    ...
    {
        if (string.IsNullOrWhiteSpace(tenantKey) || string.IsNullOrWhiteSpace(domain) || string.IsNullOrWhiteSpace(connectionName))
            return BadRequest(new { Error = "TenantKey, Domain y ConnectionName son obligatorios." });

        var rules = await ruleStore.GetAllAsync(ct);
        var filtered = rules.Where(rule =>
            string.Equals(rule.TenantKey, tenantKey, StringComparison.OrdinalIgnoreCase)
            && string.Equals(rule.Domain, domain, StringComparison.OrdinalIgnoreCase)
            && string.Equals(rule.ConnectionName, connectionName, StringComparison.OrdinalIgnoreCase))
            .OrderByDescending(rule => rule.IsActive)
            .ThenByDescending(rule => rule.UpdatedUtc)
            .ToList();

        var items = new List<object>(filtered.Count);
        ...
        CancellationToken ct = default)
    {
        ...
        var filtered = events.Where(item =>
            string.Equals(item.TenantKey, tenantKey, StringComparison.OrdinalIgnoreCase)
    ...
    [HttpPost]
    public async Task<IActionResult> CreateAsync([FromBody] UserSqlAlertUpsertRequest request, CancellationToken ct)
        => await UpsertInternalAsync(request with { Id = 0 }, ct);

    [HttpPut("{id:long}")]
    public async Task<IActionResult> UpdateAsync(long id, [FromBody] UserSqlAlertUpsertRequest request, CancellationToken ct)
        => await UpsertInternalAsync(request with { Id = id }, ct);

    [HttpPost("preview")]
    public async Task<IActionResult> PreviewAsync([FromBody] UserSqlAlertUpsertRequest request, CancellationToken ct)
    {
        try
        {
    ...
    public async Task<IActionResult> SetActiveAsync(long id, [FromBody] UserSqlAlertActivationRequest request, CancellationToken ct)
    {
    ...
    private async Task<IActionResult> UpsertInternalAsync(UserSqlAlertUpsertRequest request, CancellationToken ct)
    {
        try
        {
            var rule = ToRule(request.Id, request.RuleKey, request.TenantKey, request.Domain, request.ConnectionName, request.DisplayName, request.MetricKey, request.DimensionKey, request.DimensionValue, request.ComparisonOperator, request.Threshold, request.TimeScope, request.EvaluationFrequencyMinutes, request.CooldownMinutes, request.IsActive, request.Notes);
